...
EXPORT void *my2_SDL_CreateWindow(x86emu_t* emu, const char *title, int x, int y, int w, int h, uint32_t flags)
{
    ...
    if (force_es && *force_es && (force_es[0] != '0')) {
        ...
        if (!isdigit(force_es[0]) || (force_es[1] != '\0' && !isdigit(force_es[1]))) {
            ...
            my->SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_ES);
            my->SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, force_es[0] - '0');
            my->SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, force_es[1] ? (force_es[1] - '0') : 0);
